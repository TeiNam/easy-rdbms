...
> which blocks writers instead of failing them — so run it off peak with a `lock_timeout`.
> For a future range that has no incoming writes, the separate-table/ATTACH variant below takes a
> weaker lock on the parent.
...
**Precondition:** the default has no rows in the new range, and writers must not target that range
until attachment completes. The exclusion CHECK enforces on new rows immediately, even with
`NOT VALID`; without that precondition it would reject otherwise valid INSERTs. If the range is
already live, use the transactional move above instead.

```sql
-- Prepare the future November partition without locking the parent for a table build.
-- Identity is generated by inserts through the parent; do not copy a separate identity sequence.
CREATE TABLE log.chat_history_2026_11
  (LIKE log.chat_history INCLUDING DEFAULTS INCLUDING CONSTRAINTS INCLUDING INDEXES);
ALTER TABLE log.chat_history_2026_11 ADD CONSTRAINT chk_chat_history_2026_11_range
  CHECK (created_at >= '2026-11-01' AND created_at < '2026-12-01');

ALTER TABLE log.chat_history_default ADD CONSTRAINT chk_chat_history_default_excl_2026_11
  CHECK (created_at < '2026-11-01' OR created_at >= '2026-12-01') NOT VALID;
ALTER TABLE log.chat_history_default VALIDATE CONSTRAINT chk_chat_history_default_excl_2026_11;

BEGIN;
SET LOCAL lock_timeout = '5s';
ALTER TABLE log.chat_history ATTACH PARTITION log.chat_history_2026_11
  FOR VALUES FROM ('2026-11-01') TO ('2026-12-01');
ALTER TABLE log.chat_history_default DROP CONSTRAINT chk_chat_history_default_excl_2026_11;
COMMIT;
```

If preparation or attachment fails, the standalone table and any created exclusion CHECK remain. Keep the
target range paused and retry, or drop the helper CHECK before permitting writes to that range again.
