...
|---|---|---|---|---|
| 1 | `messages.id SERIAL` (`int`, 2.1B ceiling) | Rows are rate × time with no cap. At 10k messages/s the range is gone in **~5 days**; at a modest 100/s, ~8 months. Retention does not help — a sequence never reuses values, so deleting old rows frees storage but **not ID range** | The busiest table, at the worst moment | **Weeks.** Full table rewrite under `ACCESS EXCLUSIVE`, every index rebuilt, every referencing column migrated in lockstep, application deploys in the middle |
